...
namespace Pixel {
	...
	void PSO::SetRootSignature(Ref<RootSignature> pRootSignature)
	{
		m_pRootSignature = pRootSignature;
	}

	Ref<Pixel::RootSignature> PSO::GetRootSignature() const
	{
	...
	void GraphicsPSO::SetDepthTargetFormat(DXGI_FORMAT DSVFormat, uint32_t MsaaCount /*= 1*/, uint32_t MsaaQuality /*= 0*/)
	{
		SetRenderTargetFormats(0, nullptr, DSVFormat, MsaaCount, MsaaQuality);
	}

	void GraphicsPSO::SetRenderTargetFormat(DXGI_FORMAT RTVFormat, DXGI_FORMAT DSVFormat, uint32_t MsaaCount /*= 1*/, uint32_t MsaaQuality /*= 0*/)
	{
		SetRenderTargetFormats(1, &RTVFormat, DSVFormat, MsaaCount, MsaaQuality);
	}

	void GraphicsPSO::SetRenderTargetFormats(uint32_t NumRTVs, const DXGI_FORMAT* RTVFormats, DXGI_FORMAT DSVFormat, uint32_t MsaaCount /*= 1*/, uint32_t MsaaQuality /*= 0*/)
	{
		PX_CORE_ASSERT(NumRTVs == 0 || RTVFormats != nullptr, "null format array conflicts with non-zeron length");

		for (uint32_t i = 0; i < NumRTVs; ++i) {
			PX_CORE_ASSERT(RTVFormats[i] != DXGI_FORMAT_UNKNOWN, "Unknown Formats");
	...
	{
		m_PSODesc.pRootSignature = m_pRootSignature->GetNativeSignature();
		PX_CORE_ASSERT(m_pRootSignature->GetNativeSignature() != nullptr, "root signature is nullptr!");
