...
#include "DirectXIndexBufferView.h"
...
namespace Pixel {
	...
	void DirectXByteAddressBuffer::CreateDerivedViews()
	{
		...
		if (std::static_pointer_cast<DirectXDescriptorCpuHandle>(m_SRV)->GetCpuHandle().ptr == -1)
			std::static_pointer_cast<DirectXDescriptorCpuHandle>(m_UAV)->SetCpuHandle(DescriptorAllocator::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV));
		...
		if(TempUavHandle->GetCpuHandle().ptr == -1)
			TempUavHandle->SetCpuHandle(DescriptorAllocator::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV));
